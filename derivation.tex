...
over certain period of time. This is correct, but not in defining $R0$,
because $R0$ is a measure of how many new persons a infectious person infects
over the period he's infectious, and that phenomena is \emph{linear}. ( Later
    presented in Section~\ref{sect:discrete_time}
Fig.~\ref{fig:infection_feedback_system} ). 
...
THIS PART BELOW HAS TO BE REVISITED. IT IS INCONSISTENT WITH THE FEEDBACK MODEL OF
Chapter \ref{sect:discrete_time}

Under assumption that infected persons spread the disease with constant
fraction of cases per day, the infected persons tomorrow can be
expressed as
\begin{align}
...
These two expansion mechanisms are related, and they should match at least in early phases of
...
\begin{align}
    ...
    T_c&=\frac{R_0}{S_0}=10.
\end{align}
Thus the estimate of the contamination time is 10 days, and we assume that to
remain constant.
...
reduce $S$ in order to stop the epidemic under assumption that $T_c$ does not
...
\begin{align}
    S_lT_c&<1\\
        S_l&<\frac{S_0}{R_o}.
\end{align}
With the given values for current COVID19 outbreak $S_l<0.1$. 
...
\begin{table}[h!]
  ...
  \end{center}
\end{table}
These values reveal that with given parameter spread the contamination time,
it lies between 6.25 to 12 days. 
...
quite safe. Both the $R_0$ and $S_0$ are inaccurate, but we may make a
hypothesis that $S_l=0.1$ seems sufficient to stop the growth with $R_0\leq
3.0$
